...
angular.module('simpleLogin', ['firebase', 'firebase.utils', 'changeEmail'])
  ...
  .factory('simpleLogin', ['$firebaseAuth', 'fbutil', 'createProfile', 'changeEmail',
    function($firebaseAuth, fbutil, createProfile, changeEmail) {
      ...
      var fns = {
        ...
        createAccount: function(fullname, url, address, city, state, zipcode, email, pass, coords) {
          return auth.$createUser({email: email, password: pass})
            ...
            .then(function(user) {
              ...
              return createProfile(user.uid, email, fullname, address, city, state, zipcode, url, coords).then(function () {
                return user;
              });
            });
        },
  ...
  .factory('createProfile', ['fbutil', '$q', '$timeout', function(fbutil, $q, $timeout) {
    return function(id, email, fullname, address, city, state, zipcode, url, coords) {
      var ref = fbutil.ref('users', id), def = $q.defer();
      ref.set({email: email, name: fullname, address: address, city: city, state: state, zipcode: zipcode, url: url, coordinates:coords}, function(err) {
        $timeout(function() {
          if( err ) {
            def.reject(err);
          }
